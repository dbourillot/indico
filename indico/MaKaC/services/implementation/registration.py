...
from indico.modules.oauth.models.oauth import OAuthApplication
...
class ConferenceEticketQRCode(RegistrationModifBase):
    ...
    def _getAnswer(self):
        oauth_checkin = OAuthApplication.find_first(name='Indico Checkin')
        if oauth_checkin is None:
            raise NoReportError(_("indico-checkin is not registered as a OAuth application"))
        ...
        qr_data = {"event_id": self._conf.getId(),
                   ...
                   "server": {"baseUrl": baseURL,
                              "consumerKey": oauth_checkin.client_id,
                              "consumerSecret": oauth_checkin.client_secret,
                              }
                   }
